extends Area2D
class_name TutorialStepTrigger
...
@export var step_id := ""
@export var action: TriggerAction = TriggerAction.START_STEP
...
func _resolve_gate_to_remove() -> Node:
	...
	if action != TriggerAction.COMPLETE_STEP:
		return null
	match step_id:
		"move":
...
func _on_body_entered(body: Node) -> void:
	...
	if action == TriggerAction.START_STEP:
		_controller.start_step(step_id, prompt_text, key_labels)
	else:
